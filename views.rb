module UserInterface::Views
  ...
  def layout
    ...
    tag!(:head) do
      ...
      script do
        ...
        text "window.urlroot = #{@root.to_json()};"
        text "window.room = #{@room.to_json};" if @room
        text "window.settings = #{@settings.to_json};" if @settings
        text "window.openid = #{@state.identity.to_json};" if @state.identity
        text "window.smiliesData = #{@smilies.to_json};" if @smilies
      end
  ...
  def chat
    @headstuff = proc do
      ...
      script "window.ui_root = #{(ui_root + '/').to_json};"
  ...
  def room_settings
    @headstuff = proc do
      ...
      script "window.room = #{@room.to_json};"
